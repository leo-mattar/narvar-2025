...
function oldSiteGlobalCode() {
  var m = {
    open: function (el) {
      ...
      if (!src) {
        ...
        return; // Exit early if the video source is not available
      }
      src += "?autoplay=1";
      var modal = '<div class="modal__bg"><div class="modal__video">';
      modal += '<iframe src="' + src + '"></iframe>';
      modal +=
        '<a href="#" class="modal__close"><span></span><span></span></a>';
      modal += "</div></div>";
      $("body").append(modal);
      $("html,body").addClass("modal--active");
    },
    close: function () {
      $(".modal__bg").fadeOut(400, function () {
  ...
  $(document).ready(function () {
    $(".modal-vid-btn").on("click", function () {
      m.open($(this));
