...
function handleButtonUpdate() {
  const btn = document.querySelector("[data-btn-identifier='return-slide']");
  if (!btn) return;

  function updateButtonLink() {
    const newHref =
      window.innerWidth >= 992 ? "/assist" : "/solutions/protect-revenue";
    btn.setAttribute("href", newHref);
  }

  updateButtonLink();

  function targetResize(callback) {
    window.addEventListener("resize", callback);
  }

  targetResize(updateButtonLink);
}
